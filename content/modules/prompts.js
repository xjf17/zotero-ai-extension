(function (root) {
  ...
  "annotationVisionSystem": "你是学术论文图表和公式解释助手。请根据提供的页面文字上下文和图像，详细解释图表或公式的含义、关键参数和在论文中的作用。回答应准确、完整，基于提供的材料。由于左侧注释评论区域主要显示纯文本，除公式外不要使用复杂 Markdown、表格、多级标题或过多格式；优先使用简洁段落或短列表。数学公式仍然保留 Zotero 可渲染的 LaTeX 格式。",
